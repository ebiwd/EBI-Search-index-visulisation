...
if (!isIE11) {
  svg.selectAll("*").remove(); //empty placeholder
  d3.json("data.json", function (error, graph) {
    ...
    var tool_tip = d3.tip().attr("class", "d3-tip").offset([-8, 0]).direction(function (d, i) {
      // console.log(d.x,d.y);
      if (d.x < 580) {
    ...
    var link = svg.append("g").attr("class", "links").selectAll("line").data(graph.links).enter().append("line").attr('opacity', 0).attr("stroke-width", function (d) {
      return Math.sqrt(d.value);
    });

    var node = svg.append("g").attr("class", "nodes").selectAll(".nodes").data(graph.nodes).enter().append("g").attr("transform", function (d) {
      return "translate(" + d.x + "," + d.y + ")";
    });

    node.append("circle").attr("r", function (d) {
      return Math.sqrt(Math.sqrt(d.size / 100)) + 0;
    })
    // .attr("cx", function(d) { return d.x; })
    // .attr("cy", function(d) { return d.y; })
    .attr("fill", function (d) {
      return "rgba(255,255,255,0.0)";
    }).attr('opacity', 0).attr("stroke", function (d) {
      return color(d.group);
    }).on("mouseover", tool_tip.show).on("mouseout", tool_tip.hide).on("mousedown", function (d) {
      window.open('https://www.ebi.ac.uk/ebisearch/overview.ebi', '_blank');
    })
    ;

    // don't overlap labels
    window.collide = d3.bboxCollide(function (a) {
    ...
    var corenodes = graph.nodes.filter(function (d) {
      return d.level == "1";
    });

    // annotations only for thing other than ie11 or less
    if (!isIE11) {
      window.makeAnnotations = d3.annotation()
        .type(d3.annotationLabel)
        .annotations(corenodes.map(function (d, i) {
          // console.log(d,i);
          return {
      ...
      window.makeAnnotations = d3.annotation();
      var ie11Labels = svg.append("g").attr("class", "ie11Labels").selectAll('.ie11Labels').data(graph.nodes).enter()
    ...
    svg.append("g").attr('opacity', 0).attr("class", "annotation-group").call(makeAnnotations);

    // simulation
    var simulation = d3.forceSimulation(graph.nodes).velocityDecay(0.1).alphaDecay(.1).alpha(1)
    // .alphaTarget(.02)
    .force("x", d3.forceX().strength(0.038)).force("y", d3.forceY().strength(0.238)).force("link", d3.forceLink().id(function (d) {
      return d.id;
    }).distance(200).strength(1)).force("collide", d3.forceCollide().radius(function (d) {
      return Math.sqrt(Math.sqrt(d.size / 100)) + 1;
    }).strength(1).iterations(10)).force("center", d3.forceCenter(width / 1.7, height / 2.3)).on("tick", ticked).on("end", function () {
      var noteBoxes = makeAnnotations.collection().noteNodes;
      window.labelForce = d3.forceSimulation(noteBoxes).force("x", d3.forceX(function (a) {
        return a.positionX;
      }).strength(function (a) {
        return Math.max(0.25, Math.min(3, Math.abs(a.x - a.positionX) / 20));
      })).force("y", d3.forceY(function (a) {
        return a.positionY;
      }).strength(function (a) {
        return Math.max(0.25, Math.min(3, Math.abs(a.x - a.positionX) / 20));
      })).force("collision", window.collide).alpha(0.5).on('tick', function (d) {
        makeAnnotations.annotations().forEach(function (d, i) {
          var match = noteBoxes[i];
    ...
    simulation.force("link").links(graph.links).distance(10);

    // make things visible
    d3.selectAll("circle").transition(t).delay(1000).attr("opacity", 1);

    d3.selectAll(".legendSize").transition(t).delay(1900).attr("opacity", .5);

    d3.selectAll(".annotation-group").transition(t).delay(function (d) {
  ...
  svg.append("g").attr("class", "legendSize").attr("stroke", "#fff").attr("fill", "#000").attr("transform", "translate(" + (width - 250) + ", " + (height - 150) + ")");

  var legendSize = d3.legendSize().scale(linearSize).shape('circle').shapePadding(2).labelFormat('.0s').cells(5).title('Millions of entries in each domain').labelOffset(15).orient('horizontal');

  svg.select(".legendSize").attr("opacity", "0").call(legendSize);

  svg.select(".legendSize text.legendTitle").attr("transform", function (d) {
  ...
  function customStratify(data) {
    ...
    for (i = 0; i < len; i += 1) {
      ...
      for (var i2 = 0; i2 < data[i].subdomains.domain.length; i2 += 1) {
